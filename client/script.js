...
const createFood = event => {
    ...
    .then(res => {
        console.log(res.data);
        let foodList = document.createElement('h2');
        foodList.classList.add('food-list');
        foodList.textContent = body.newFood;
        document.querySelector('body').appendChild(foodList);
    })
    foodInput.value = '';
}

document.querySelector('#foodbutton').addEventListener('click', createFood);